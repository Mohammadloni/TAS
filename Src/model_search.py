...
class Cell(nn.Module):
  ...
  def __init__(self, steps,layer_no ,multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    if reduction_prev:
      ...
      self.preprocess0 = BinReLUConvBN(C_prev_prev, C, 1, 1, 0, affine=False)
    self.preprocess1 = BinReLUConvBN(C_prev, C, 1, 1, 0, affine=False)
    if self.reduction:
...
class Network(nn.Module):
  ...
  def genotype(self, gamma=1.):
    ...
    gene_reduce = _parse(F.softmax(self.alphas_reduce, dim=-1).data.cpu().numpy(), PRIMITIVES_parse = PRIMITIVES_reduce)
