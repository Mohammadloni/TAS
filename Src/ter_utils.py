...
class TerOp():
    def __init__(self, model, args):
        ...
        for n,m in model.named_modules():
            if isinstance(m, nn.Conv2d):
                index = index + 1
                if index in self.ter_range and 'res' not in n and index >= (self.num_skip + 1):
    ...
    def initial_scailing_param(self):
        for index in range(self.num_of_params):
            ...
            self.initial_scaling_factors += Variable(torch.FloatTensor([w_p_initial, w_n_initial]).cuda(),
                                                        requires_grad=True)
    ...
    #         # print(delta.shape)
    #         # alpha = self.Alpha(weight,delta)
    #         n = weight[0].nelement()
    #         s = weight.size()
    #         m = weight.norm(1, 3, keepdim=True)\
    #                 .sum(2, keepdim=True).sum(1, keepdim=True).div(n).expand(s)
    #         for i in range(weight.size()[0]):
    #             m[i][weight[i].lt(-delta[i])] = 0
    #             m[i][weight[i].gt(delta[i])] = 0
    #
    #         m = m.mul(self.target_modules[index].grad.data)
    #         m_add = self.sign_new(copy.deepcopy(weight),delta.view(-1,1),alpha).mul(self.target_modules[index].grad.data)
    #         m_add = m_add.sum(3, keepdim=True)\
    #                 .sum(2, keepdim=True).sum(1, keepdim=True).div(n).expand(s)
    #         m_add = m_add.mul(self.sign_new(copy.deepcopy(weight),delta.view(-1,1),alpha))
    #         self.target_modules[index].grad.data = m.add(m_add).mul(1.0-1.0/s[1]).mul(n)
    #         A = weight.norm(1, 3, keepdim=False).sum(2, keepdim=False).sum(1, keepdim=False).div(self.delta_w)
    #         B = self.target_modules[index].grad.data.norm(1, 3, keepdim=False).sum(2, keepdim=False).sum(1, keepdim=False).div(n)
    #         self.delta_w.grad=torch.ones(1,requires_grad=True,device="cuda")*torch.mean(A * B)
    # def updateTernaryGradWeight(self):
    #     for index in range(self.num_of_params):
    #         weight = self.target_modules[index].data
...
#     output = torch.zeros_like(tensor)
#     for i in range(tensor.size()[0]):
#         output[i] = torch.where(torch.logical_and(tensor[i] >= -delta[i], tensor[i] <= delta[i]), torch.tensor(0.0), alpha[i])
#         output[i] = torch.where(tensor[i] < -delta[i], -alpha[i], output[i])
#
#     return output
# if __name__ =="__main__":
#     x= torch.randn(4,1,2,2)*3
#     delta = torch.ones(4,1)*0.2
#     alpha = torch.tensor([1,2,3,4])*0.1
#     print(sign_new(x,delta,alpha))