...
def main():
  ...
  ter_op =ter_utils_search.TerOp(model, args)
  optimizer_delta_w = torch.optim.SGD(
    [ter_op.delta_w],
    args.learning_rate,
    momentum=args.momentum,
    weight_decay=args.weight_decay)
  optimizer_scailing_factor= torch.optim.SGD(
    ter_op.initial_scaling_factors,
    args.learning_rate,
    momentum=args.momentum,
    weight_decay=args.weight_decay)
  best_acc = 0.
  ...
  for epoch in range(args.epochs):
    ...
    # training
    train_acc, train_obj = train(train_queue, valid_queue, model, architect, criterion, optimizer,optimizer_delta_w,optimizer_scailing_factor, lr, ter_op, epoch)
    logging.info('train_acc %f', train_acc)

    # validation
    valid_acc, valid_obj = infer(valid_queue, model, criterion, ter_op)
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer,optimizer_delta_w,optimizer_scailing_factor, lr, ter_op, epoch):
  objs = utils.AvgrageMeter()
  top1 = utils.AvgrageMeter()
  top5 = utils.AvgrageMeter()
  model.train()
  for step, (input, target) in enumerate(train_queue):
    ter_op.ternarization()
    ...
    optimizer.zero_grad()
    optimizer_delta_w.zero_grad()
    optimizer_scailing_factor.zero_grad()
    logits = model(input)
    loss = criterion(logits, target)

    loss.backward()
    ter_op.restore()
    ter_op.updateTernaryGradWeight()
    optimizer.step()
    optimizer_delta_w.step()
    optimizer_scailing_factor.step()
...
def infer(valid_queue, model, criterion, ter_op):
  objs = utils.AvgrageMeter()
  top1 = utils.AvgrageMeter()
  top5 = utils.AvgrageMeter()
  model.eval()
